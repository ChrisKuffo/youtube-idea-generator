...
import { Idea } from "@/server/db/schema"; // Idea type from database schema
...
import {
  ...
  getNewIdeas,
} from "@/server/ideas-actions"; // Functions for idea actions
...
import YoutubeLogo from "@/public/youtube-logo.png"; // YouTube logo image

// Props interface for the component
interface Props {
  initialIdeas: Idea[]; // Initial list of ideas
}

// Interface for idea details
...
export default function IdeaList({ initialIdeas }: Props) {
  const [ideas, setIdeas] = useState<Idea[]>(initialIdeas); // State for ideas
  ...
  const [ideaDetails, setIdeaDetails] = useState<Record<string, IdeaDetails>>(
    {}
  ); // State for idea details
  const { toast } = useToast(); // Toast notification hook

  // Ref to keep track of previous unprocessed jobs state
  ...
  const handleGenerate = async () => {
    ...
    try {
      await kickoffIdeaGeneration(); // Start idea generation process
      toast({
  ...
  useEffect(() => {
    const pollJobsInterval = setInterval(async () => {
      try {
        await processPendingJobs(); // Process any pending jobs

        const hasUnprocessedJobs = await checkForUnprocessedJobs(); // Check for unprocessed jobs
    ...
    const fetchDetailsForIdeas = async () => {
      for (const idea of ideas) {
        if (!ideaDetails[idea.id]) {
          const details = await getIdeaDetails(idea.videoId, idea.commentId); // Fetch details for each idea
          setIdeaDetails((prev) => ({
            ...prev,
            [idea.id]: details,
          }));
  ...
  // Render when there are no ideas
  if (ideas.length === 0) {
    return (
